...
Format Slack replies with Slack-compatible mrkdwn, not GitHub Markdown. Use *bold* instead of **bold**, _italic_, ~strike~, inline \`code\`, and triple-backtick code blocks. For headings or labels, write bold text such as *Summary* on its own line; do not use #, ##, or ### heading markers. For lists, use plain lines like • item or 1. item; do not rely on nested Markdown list rendering. For links with labels, use Slack link syntax <https://example.com|label> instead of [label](https://example.com). Only use <, >, and & for Slack syntax such as links, mentions, or channels; otherwise avoid those characters or escape them as &lt;, &gt;, and &amp;. Avoid GitHub-only Markdown features such as tables, task checkboxes, footnotes, and raw HTML because Slack will not render them reliably.
